...
# works (tested) only for internal drives (disk2)
# assumes default config: disk2s2 is Restore11 (current boot volume).
# will erase all other partitions 
# will create named 'leegsplit' on disk2s3 with room for recovery HD
...
LISTOFSLICES=`diskutil list | grep : | grep disk2s | awk '{ print $NF; }'`
...
echo " - CoreStorage found, deleting CS volume"
	...
	echo " - done erase LVG"
	LAST_SLICE=`diskutil list|grep : |grep disk2s | awk ' END { print $NF; }'`
#	echo " - LAST_SLICE (cs) is : " $LAST_SLICE
	echo " - first merge $DISKUTIL_CS to $LAST_SLICE"
	echo y | diskutil mergePartitions force JHFS+ leeg disk2s3 $LAST_SLICE
	diskutil splitpartition disk2s3  JHFS+ leegsplit R %Apple_Boot% %noformat% %recovery% 2>&1
# looks silly to do split twice, but otherwise the last partition will be 2, 3 times 650 MB
else
	echo " - No CoreStorage volumes found"
fi

LISTOFSLICES=`diskutil list|grep disk2s | awk '{ print $NF; }'`
echo " - LISTOFSLICES is : " $LISTOFSLICES
LAST_SLICE=`diskutil list|grep disk2s | awk 'END { print $NF; }'`
echo " - LAST_SLICE is : " $LAST_SLICE

	echo " - Proceed to merge disk2s3 (again for CS) to $LAST_SLICE"
	echo y | diskutil mergePartitions force JHFS+ leeg disk2s3 $LAST_SLICE
	echo " - done merging"

echo " - creating room for recovery at end of disk2s3"
diskutil splitpartition disk2s3  JHFS+ leegsplit R %Apple_Boot% %noformat% %recovery% 2>&1
